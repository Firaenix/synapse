...
export class TorrentManager {
  ...
  public addTorrent = async (metaInfo: MetainfoFile, keyPair?: KeyPair, fileChunks?: Array<Buffer>) => {
    if (!metaInfo) {
    ...
    await this.peerManager.searchByInfoIdentifier(this.metainfoService.infoIdentifier);
  };
  ...
  private isPieceValid = async (index: number, offset: number, pieceBuf: Buffer): Promise<boolean> => {
    ...
    const pieceHash = await this.hashService.hash(pieceBuf, algo);
  ...
  private requestPiecesLoop = async () => {
    try {
      ...
      const isPieceValid = await this.isPieceValid(pieceIndex, pieceOffset, pieceBuf);
      if (isPieceValid === false) {
